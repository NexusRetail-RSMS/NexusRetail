...
didSet { savePerUserPreference() }
    ...
    var currentUserId: UUID? {
        didSet {
            // Load the preference for the new user immediately
            isDarkMode = UserDefaults.standard.bool(forKey: storageKey())
        }
    }
    
    // Backwards-compatible helper accepting String IDs used by older call sites
    func configure(for userId: String?) {
        currentUserId = userId.flatMap { UUID(uuidString: $0) }
        isDarkMode = UserDefaults.standard.bool(forKey: storageKey())
    }
    
    private func storageKey() -> String {
        if let uid = currentUserId?.uuidString { return "isDarkMode_\(uid)" }
        return "isDarkMode"
    }
